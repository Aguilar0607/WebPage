

<footer class="bg-white rounded-lg shadow dark:bg-gray-900 m-4">
    <div class="w-full max-w-screen-xl mx-auto p-4 md:py-8">
        <div class="sm:flex sm:items-center sm:justify-between">
            <a href="/sitio/Paguina/WebPage/index" class="flex items-center mb-4 sm:mb-0">
                <img src="/sitio/Paguina/WebPage/assets/img/logos/olpega-logo-blue.svg" class="h-8 mr-3" alt="Olpega Logo" />
            </a>
            <ul class="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
                <li>
                    <a href="https://olpega.net/soporte/" class="mr-4 hover:underline md:mr-6 ">Soporte tecnico</a>
                </li>
                <li>
                    <a href="https://olpega.net/sitio/assets/img/PO-07-01-02%20AVISO%20DE%20PRIVACIDAD%20CCTV%20V01.pdf" class="mr-4 hover:underline md:mr-6">Aviso de privacidad CCTV</a>
                </li>
                <li>
                    <a href="https://forms.office.com/r/BP9Hv0nVNu" class="mr-4 hover:underline md:mr-6 ">Buzon de quejas</a>
                </li>
            </ul>
        </div>
        <hr class="my-6 border-gray-200 sm:mx-auto dark:border-gray-700 lg:my-8" />
        <span class="block text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2023 <a href="/sitio/Paguina/WebPage/" class="hover:underline">Distribuidora Olpega SA de CV </a> - Todos los derechos revervados.</span>
    </div>
</footer>



    <script type="text/javascript" src="/sitio/Paguina/WebPage/node_modules/flowbite/dist/flowbite.min.js"></script>
    <script type="text/javascript "src="/sitio/Paguina/WebPage/assets/js/kutty.min.js"></script>
    
    

</body>

</html>
